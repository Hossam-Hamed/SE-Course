angular.module('App')
	.factory("outserv" ,function($http,$scope){
       return {
	...
	get: function(landing){
		console.log(scope.landing.origin,$scope.landing.destination)
		return $http.get('http://localhost:3000/api/flights/search/'
			+scope.landing.origin+'/'+$scope.landing.destination+'/'+$scope.landing.departingDate+'/'+$scope.landing.cabin),
			{
